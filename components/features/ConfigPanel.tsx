        ...
        <div className="flex p-1 bg-slate-100/80 rounded-lg">
          {[
            ...
            <button
              ...
              className={`flex-1 py-1.5 text-xs font-semibold rounded-md transition-all duration-200 ${activeTab === tab.id
            ...
            <div className="animate-in fade-in duration-300 space-y-6">
              ...
              <Section title="Brand Identity" description="Logo and company name">
                <div className="flex items-start gap-4">
                  <div
                    className={`w-16 h-16 rounded-xl border ${uploadError ? 'border-red-300 bg-red-50' : 'border-slate-200 bg-slate-50'} flex items-center justify-center cursor-pointer hover:border-blue-400 hover:bg-blue-50/30 transition relative overflow-hidden group shrink-0`}
                    onClick={() => fileInputRef.current?.click()}
                  >
                    {profile.logoUrl ? (
                      <img src={profile.logoUrl} alt="Logo" className="w-full h-full object-contain p-2" />
                    ) : (
                      <Upload className={`w-5 h-5 ${uploadError ? 'text-red-400' : 'text-slate-400'}`} />
                    )}
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    />
                    {uploadError ? (
                      <p className="text-[10px] text-red-500 font-bold animate-pulse">{uploadError}</p>
                    ) : profile.logoUrl && (
                      <button
              ...
              {/* Signature Section */}
              <Section title="Digital Signature" description="Add a signature to the bottom of your letter">
                <div className="flex items-start gap-4">
                  <div
                    className="w-32 h-16 rounded-xl border border-slate-200 bg-slate-50 flex items-center justify-center cursor-pointer hover:border-blue-400 hover:bg-blue-50/30 transition relative overflow-hidden group shrink-0"
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (file) {
                          const reader = new FileReader();
                          reader.onloadend = () => setProfile(prev => ({ ...prev, signatureUrl: reader.result as string, showSignature: true }));
                          reader.readAsDataURL(file);
                        }
                      }}
                    />
                  ...
                  <div className="flex-1 space-y-2 pt-1">
                    <Toggle
                      ...
                      checked={profile.showSignature}
                      onChange={(v) => setProfile(prev => ({ ...prev, showSignature: v }))}
                    />
                    {profile.signatureUrl && (
                      <button
                        onClick={() => setProfile(prev => ({ ...prev, signatureUrl: null }))}
                        className="text-[10px] text-red-500 hover:text-red-600 flex items-center gap-1 mt-1"
                      >
              ...
              <button
                ...
                className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-bold transition-all ${saveStatus === 'saved'
                ...
                <div className="grid grid-cols-2 gap-2">
                  {['modern', 'classic', 'minimal', 'executive', 'creative', 'bold'].map((l) => (
                    <button
                      ...
                      className={`px-3 py-3 rounded-lg text-xs font-medium capitalize border transition-all text-center ${profile.layout === l
                    ...
                    <div className="grid grid-cols-1 gap-1">
                      {[
                        ...
                        <button
                          ...
                          className={`px-3 py-2 rounded-md text-left text-xs flex items-center justify-between transition-all ${profile.fontFamily === font.id
